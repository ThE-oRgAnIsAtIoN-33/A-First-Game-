...
import pygame
from model.tile_dict import GetDict
...
class Level():
    ...
    def SetLevel(self, level_number):
        ...
        if level_number == 1:
            #set text
            level_file_name = "model/level_" + str(level_number) + ".txt"
    ...
    def CheckForTile(self, surounding_tile, current_tile):
        ...
        if current_tile == "0":
            return (pygame.image.load("model/tile set game/air.png").convert())
        elif current_tile == "1":
            prefix = "model/tile set game/dirt_"
